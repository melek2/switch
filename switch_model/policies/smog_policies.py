...
def load_inputs(model, switch_data, inputs_dir):
    """
    Load per-generator PM2.5 cost data (in $/ton) from gen_pm25_costs.csv.
    Expected file format (with header):
      GENERATION_PROJECT,pm25_cost_dollar_per_ton
    """
    switch_data.load_aug(
        filename=os.path.join(inputs_dir, "gen_pm25_costs.csv"),
...
def post_solve(model, outdir):
    """
    Export annual PM2.5 metrics to smog.csv
    """
